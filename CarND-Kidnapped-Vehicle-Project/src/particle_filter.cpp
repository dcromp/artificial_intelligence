...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
	...
	for (int i = 0; i < observations.size(); ++i) {
		...
		int smallest_id = -1;
		//cout << "Observation location x: " << observation.x << " y: "<<  observation.y << endl;

		for (int i2 = 0; i2 < predicted.size(); ++i2) {

			LandmarkObs predict = predicted[i2];

			double distance = dist(observations[i].x, observations[i].y, predict.x, predict.y);

			if (distance < smallest_dist) {
				smallest_dist = distance;
				smallest_id = predict.id;
			}

		}

		observations[i].id = smallest_id;
	}
}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	// define inputs for multi guassian
	...
	for (int i = 0; i < num_particles; ++i) {
		...
		// transform observations to map coordinates with respect to the particle
		std::vector<LandmarkObs> map_observations;
		for (int h = 0; h < observations.size(); ++h) {
			LandmarkObs observation = observations[h];

			double x_map = p_x + (cos(particle.theta) * observation.x) - (sin(p_theta) * observation.y);
			double y_map = p_y + (sin(particle.theta) * observation.x) + (cos(p_theta) * observation.y);
			map_observations.push_back(LandmarkObs{ h, x_map, y_map });
		}

		// Get the landmarks within the sensor_range
		...
		// assign each observation to its closest landmark
		...
		for (int k = 0; k < map_observations.size(); ++k) {
			...
			// define inputs for multi guassian
			double x_obs = map_observations[k].x;
			double y_obs  = map_observations[k].y;


			double mu_x;
			double mu_y;
			int location = map_observations[k].id;

			for (int l = 0; l < predicted.size(); ++l) {
				if (location == predicted[l].id) {
					mu_x = predicted[l].x;
					mu_y = predicted[l].y;
				}
			}
      ...
      double obs_w = ( 1/(2*M_PI*s_x*s_y)) * exp( -( pow(mu_x-x_obs,2)/(2*pow(s_x, 2)) + (pow(mu_y-y_obs,2)/(2*pow(s_y, 2))) ) );
...
void ParticleFilter::resample() {
  ...
  int index = unidist(gen);
	...
	for (int i = 0; i < num_particles; ++i) {
		beta += ((double) rand() / (RAND_MAX)) * 2 * max_weight;
